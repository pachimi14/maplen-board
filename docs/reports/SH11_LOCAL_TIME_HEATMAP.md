...
ブラウザ実機(`Asia/Tokyo`)で統括が測定。

## (a)(c) ローカル時刻 + 曜日 + タイムゾーン明示

```
X軸        : 07/03 (金) / 07/14 (火) / 07/24 (金) / 08/05 (水)
計算条件   : 表示時刻: お使いの端末のタイムゾーン(UTC+9)
             履歴の最終更新: 2026-08-05 05:00 (水)   ← UTC 2026-08-04 20:00 の JST 表現
             現在価格の取得時刻: 2026-08-05 13:00 (水)
```

## (b) 曜日名のロケール追随(`Intl`・ハードコード表なし)

| ロケール | X軸 | タイムゾーン行 |
|---|---|---|
| ja | `08/05 (水)` | 表示時刻: お使いの端末のタイムゾーン(UTC+9) |
| en | `08/05 (Wed)` | Displayed time: your device's timezone (UTC+9) |
| zh-TW | `08/05 (週三)` | 顯示時間:您裝置的時區(UTC+9) |

## (d)(e)(f) ヒートマップ — **統括が独立に再計算して完全一致**

`Chaos Von Bon Helmet`(1003720)/ ☆0→22 / JST で統括が別実装で集計:

```
セル数 42 / n合計 898(= 確定点899 − 暫定1)
日曜行  01:00=380M(21) 05:00=363M(21) 09:00=440M(21) 13:00=430M(21) 17:00=435M(22) 21:00=419M(22)
画面    01:00=380M(21) 05:00=363M(21) 09:00=440M(21) 13:00=430M(21) 17:00=435M(22) 21:00=419M(22)
                                                                     ↑ 完全一致
最高セル: 日曜 09:00 = 440M(画面の「最高」マーカーと一致)
最安セル: 木曜 05:00 = 323M
セル間の開き: 36.2%
```

**n 合計が確定点数と一致 = 取りこぼしゼロかつ暫定点が入っていない**((d)(f) 同時に満たす)。

列見出しが `01/05/09/13/17/21` になっているのは、**UTC の4時間境界を JST で表現**したもの
(UTC 16,20,00,04,08,12 → JST 01,05,09,13,17,21)。**丸めずに実際の時刻を出す**という §2 の規約どおり。

## (g)(h)(i)(j)

- 期間タブ非連動: ラベル「過去150日全体の傾向(期間タブとは連動しません)」を表示。
  `WeekdayHeatmap` が `period` を受け取らない構造で担保
- **`server/` の差分ゼロ**(`git diff --stat -- server/` が空)
- `npm run test` 414 passed / `npm run build` 成功 / 6ロケール 388キー一致
- SH-7〜SH-10 の性質は無改変

## 評価すべき点(実装担当)

**i18n の追加で JSON 全体を再シリアライズしたところ、無関係な既存キー
(`shareImage.goalHint`)のエスケープ表記が変わる実質差分が出たことを自分で検出し、
その版を破棄してピンポイント編集でやり直した。**
これは「触っていないはずの行が動く」類の混入で、**レビューをすり抜けやすい**。
自ら気づいて捨てた判断を評価する。

## 残す制約(block しない)

- **DST のあるタイムゾーン**では、期間内に夏時間の切替を跨ぐと
  ヒートマップの列見出しが一部区間の実オフセットと最大1時間ずれうる(実装担当の自己申告)。
  **JST など DST の無い地域では発生しない**。対処するなら列見出しを
  「UTC 基準の枠」として併記する等だが、**現時点では実害が限定的なので据え置き**
- **U8(ヒートマップの期間連動)** は計画どおり未決のまま

---

## ★追記(2026-08-05・IMPL_PLAN_SH14)— 本スライスの「ローカル時刻」方針は反転された

本文(上記)はそのまま残す(旧文は書き換えない)。**ユーザー裁定により、本スライスが導入した
「ページ全体を閲覧者のローカル時刻に」表示する方針は IMPL_PLAN_SH14 で UTC に戻された。**
`localTimeZone()` / `formatTimeZoneLabel()` および `{ timeZone }` オプションはすべて削除済み。
曜日の併記自体は維持(ただし UTC 基準で算出)。反転の理由・実測・受け入れ結果は
`docs/reports/SH14_UTC_AND_ORDER.md` を参照。設計書 §15 に `U9` として同旨を記録した。
