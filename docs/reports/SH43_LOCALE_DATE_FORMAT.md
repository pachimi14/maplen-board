...
**★2026-08-22 追記(検収後の統括裁定)**: 統括の実機検収(ja/en/th)で合格した後、実装担当が
「ヒートマップ列見出しの24時間制据え置き」について裁定を求めたところ、**時刻は全ロケール
24時間制に統一(`hour12: false` を明示)** というユーザー裁定が出た。以下の本文(1本目の
コミット `d9c4de8` の記録)は当時のまま残し、その裁定内容・実測は末尾の
**「## SH-43 追加修正(2026-08-22): hour12 を全ロケール24時間制に統一」**節に追記する
(この repo の「旧文は書き換えない」規約 -- SH-11 addendum と同じ扱い)。
**(a)/(g) の下表(12時間制混在時点の実測)は歴史的記録として残すが、現行の実際の表示は
末尾の追記セクションを見ること。**
...
## SH-43 追加修正(2026-08-22): hour12 を全ロケール24時間制に統一

**裁定の経緯**: 上記本文コミット(`d9c4de8`)は統括検収で ja/en/th 合格したが、実装担当が
「ヒートマップ列見出しは24時間制のまま据え置いた」判断について統括に裁定を求めた。統括の
実機実測で、同じ画面内で **ツールチップ(`08:39 AM UTC`, 12時間制)とヒートマップ列見出し
(`09:00 UTC`, 24時間制)が混在**していることが判明。∴ **ユーザー裁定: 時刻表記は全ロケール
24時間制で統一する。日付の順序・仏暦・曜日・UTC の明示は変えない**(実装担当自身の
「ヒートマップは24時間制のまま」という判断のほうが正しく、ツールチップ側をそちらに合わせた)。

### コミット

新規コミット1本(`d9c4de8` は書き換えていない)。触った1ファイル(`format.js`。テスト・本報告
含め計3ファイル)のみ個別 `git add`。`git add -A` 不使用・`push` 不使用。

### 変更内容

`format.js` の `localizedDateString`(`formatAxisDate`/`formatTooltipDate`/
`formatTooltipDateLocal` が共有する内部ヘルパー)に `withTime` 時のみ `hour12: false` を
明示追加。**1行の変更**(`{ hour: "2-digit", minute: "2-digit" }` → `{ hour: "2-digit",
minute: "2-digit", hour12: false }`)。日付の年月日順・曜日位置・暦(`calendar` 未指定→タイは
仏暦のまま)は無改変 -- `hour12` 以外は何も触っていない。

`formatClockTime` / `formatLocalClockTime` / `formatBucketRange`(ヒートマップ列見出し・
区間注記)は**変更不要**(元から `Intl` を経由しない固定24時間制のゼロ埋め文字列のため)。
実装担当の元の判断(狭いヒートマップグリッドでの据え置き)がそのまま正しかった。

### (l) ★6ロケールとも24時間制であることの実測

同一瞬間 `2026-08-18T04:55:00Z`(UTC 火曜)、`formatTooltipDate`:

| ロケール | 修正前(12h混在) | 修正後(24h統一) |
|---|---|---|
| ja | `2026/08/18(火) 04:55 UTC` | `2026/08/18(火) 04:55 UTC`(変化なし、元から24h) |
| en | `Tue, 08/18/2026, 04:55 AM UTC` | **`Tue, 08/18/2026, 04:55 UTC`**(AM 消滅) |
| zh-TW | `2026/08/18（週二） 上午04:55 UTC` | **`2026/08/18（週二） 04:55 UTC`**(上午 消滅) |
| th | `อ. 18/08/2569 04:55 UTC` | `อ. 18/08/2569 04:55 UTC`(変化なし、元から24h) |
| vi | `04:55 Th 3, 18/08/2026 UTC` | `04:55 Th 3, 18/08/2026 UTC`(変化なし、元から24h) |
| es | `mar, 18/08/2026, 04:55 UTC` | `mar, 18/08/2026, 04:55 UTC`(変化なし、元から24h) |

**6ロケールすべて24時間制で揃った**(ja/th/vi/esは元から無変化、en/zh-TWのみAM/PM・上午/下午が消えた)。

観測時刻・ローカル行(`formatTooltipDateLocal`、`Asia/Tokyo` 指定、`2026-08-04T11:00:00Z` → JST `20:00`):

| ロケール | 修正前 | 修正後 |
|---|---|---|
| en | `Tue, 08/04/2026, 08:00 PM JST` | **`Tue, 08/04/2026, 20:00 JST`** |
| ja | `2026/08/04(火) 20:00 JST` | `2026/08/04(火) 20:00 JST`(変化なし) |

ヒートマップ列見出し(`formatClockTime`、`WeekdayHeatmap.jsx`、無改変・参考値):
`00:00` `04:00` `08:00` `12:00` `16:00` `20:00`(常に24時間制、全ロケール共通)。

### (m) AM/PM/上午/下午 が出ないことのテスト固定

`format.test.js` に2件追加:

```
it("(m) never renders AM/PM (en) or 上午/下午 (zh-TW), across every 4h bucket hour", ...)
  → 00/04/08/12/16/20 UTC の全バケット時刻で en に /AM|PM/i、zh-TW に /上午|下午/ が
    出ないことを固定(正午・深夜0時など12h/24hの境界も含む)

it("(m) never renders AM/PM (en) even for an afternoon local hour", ...)
  → formatTooltipDateLocal(Asia/Tokyo, 20:00相当) でも AM/PM が出ないことを固定
    (12h表記なら "8:00 PM" になるはずの時刻で検証)
```

### (n) 日付の順序・仏暦・曜日・UTC の明示が(a)〜(d)のまま

上表の通り、変わったのは各文字列内の**時刻部分の表記(AM/PM・上午/下午の有無)のみ**。
年月日の並び(en=月/日/年、es/vi=日/月/年、th=仏暦2569)・曜日の位置・末尾の `UTC`/
ゾーンラベルはいずれも1文字も変わっていない(上表で該当箇所を確認可能)。

### (o) ツールチップとヒートマップ列見出しの時刻表記の一致

両方とも `HH:MM`(24時間制・ゼロ埋め)で統一。例: ツールチップ `04:55 UTC` / ヒートマップ列
`04:00 UTC`(← 実際は4時間バケットの境界時刻でツールチップと厳密には一致しないが、**表記の
「制」(24時間制・ゼロ埋め・AM/PM無し)は完全に一致**)。
...
`localizedDateString` は `hour12` の1オプションのみ変更しており、`buildWeekdayHeatmap` /
`series.js` / `chartColumns.js` / `priceGapFill.js` / `server/` はいずれも今回も触っていない
(前回コミットと同じ理由で自明に不変 -- 表示文字列の1オプションを増やしただけで、
`isoDate`/`timeZone` の受け渡し方は一切変えていない)。

### 停止条件チェック

該当なし。1行の追加(`hour12: false`)のみで、狭い変更範囲のまま完了。
