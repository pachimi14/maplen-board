...
| 基準 | 実測 |
|---|---|
| (a) UTC 統一 | ヒートマップ見出し「**曜日 × 時間帯(UTC)**」/ 列 `00:00 04:00 08:00 12:00 16:00 20:00` |
| | X軸 `07/14 (火) / 07/24 (金) / 08/05 (水)`(2026-08-05 は UTC で水曜=正) |
| | **ローカル時刻の痕跡なし**(「端末のタイムゾーン」「UTC+9」の文字列が画面に0件) |
| (c) 起点 | **最上段が「木」**、左上セル = **木曜 00:00 UTC = 373.38M** |
| (f) 並び | 候補30件が `localeCompare` 昇順(`isSorted: true`)。先頭 `AbsoLab Ancient Bow / AbsoLab Archer Cape / ...` |
| (g) | `server/` の差分ゼロ(4コミットとも) |

## ★孤児コードの破棄(統括が処理)

検収時、worktree に **`server/sf-history/fetch_latest.py` の未コミット変更(86行追加・テストゼロ)**が残っていた。
**中断されたエージェントの途中生成物**で、しかも参照している「IMPL_PLAN_SH14 §2」は
**現在は別内容(UTC統一)に書き換わっている**。

∴ **`git checkout --` で破棄した。** 当該機能(20分スタンプ/キャッシュ)は
**SH-15 で計画・テストを揃えて作り直す**。

> **教訓**: 中断したエージェントの作業ツリーは、次のスライスの前に必ず `git status` で確認する。
> 未コミットの孤児が残ると、**後続コミットに黙って紛れ込む**。
> 実装担当が完了報告でこれを申し送ってきたのは正しい行動。

## 決定の反転として記録

**SH-11「ページ全体を閲覧者のローカル時刻に」を本スライスで反転した。**
反転を支持する実測: 曜日別中央値の開きは **UTC 13.9% / JST 10.4%** で、
**UTC のほうが周期が鋭い**(ゲーム側の UTC 基準リセットに紐づく示唆)。
∴ 分析の枠としても UTC が妥当。SH-11 の作業の一部は捨てたことになるが、
**より良い枠に移すための捨て**であり、記録として残す。
