...
| # | 前提 | 状態 |
|---|------|------|
| P1 | 公式 1時間足 API `dynamicpricing/enhance-price/history` が直近150日を返す | **訂正済(下記 §2.1)**。保持期間は **150日ではなく最低160日**。1リクエストで一括、ページング無し |
...
### 2.1 ★P1 の訂正 — 「窓を広く要求するほど多く返る」は成り立たない

**SH-2 の取得データを統括が検分して判明**(SH-1 の1装備・1条件では見えなかった):

| 要求窓 | 得られた点数 | 最古 |
|---|---|---|
| **200日**(SH-1) | 3510点 | `2026-03-08T01:00Z`(**149.7日前**) |
| **160日**(SH-2) | **3753点** | `2026-02-25T19:00Z`(**160.0日前 = 要求窓の端ちょうど**) |

**より広い窓を要求したほうが、返るデータが少なかった。** ∴
1. **保持期間は150日ではなく、最低でも160日ある**(P1 の「150日」は SH-1 の単一条件の観測を一般化した誤り)
2. **窓が広すぎると API が静かに少なく返す**(上限クランプかフォールバックか、機序は未解明)

**設計への影響**:
- 表示期間は **150日のまま**(要件は満たされている。160日窓で取れば余裕がある)
- **取得窓は 160日を既定とし、「広くすれば安全」と考えない。**窓を変えるときは
  **必ず返却点数と最古時刻を実測して比較する**(広げて減っていないことの確認込み)
- **真の保持期間を探る調査はしない**(表示150日に不要=作らないリスト)。必要になったら **U7** で扱う

### 2.2 星ごとに履歴の深さが違う

`item_id=1012757`(Twilight Mark)は 22段階のうち**最古時刻が6種類に割れる**(最小 3525点)。
∴ **「同一装備なら全星が同じ期間そろう」という仮定は置けない**。
§9.1 の欠損規約(`requiredPriceStars` が揃わない時点は `null`・補間しない)がそのまま効くので**設計変更は不要**だが、
**SH-3 の 4h 導出と SH-5 のチャートは「星ごとに系列の開始時刻が違う」ことを前提に書く**こと。
...
除外は `1113282`(Noble Ifia's Ring)/ `1122254`(Mechanator Pendant)。
**除外の出典は「ユーザーが原案で明示指定した2件」**であって、maplenEnhancebot 側の判断ではない。
SH-2 の実装担当が `sf_history_items.json` に書いた `reason` は**実測からの再構成**なので、
**SH-3 で出典を上記に訂正する**(推測の理由を正典に残さない)。

## 7.1 ★装備ごとに☆上限が違う(SH-2 で判明・**UI の必須要件**)

150日分を取り切った結果、**6装備は `itemUpgrade=20/21` のデータが1点も存在しない**:

| ☆上限 | 装備数 | 該当 |
|---|---|---|
| **☆20**(`itemUpgrade` 最大 19) | **6** | Black Bean Mark / Dea Sidus Earring / Royal Von Leon の Boots・Hands・Cape・Glorier |
| ☆22(`itemUpgrade` 最大 21) | 22 | 上記以外 |

これは欠損ではなく**その装備が☆20 までしか強化できない**という事実。

**∴ UI は装備ごとに目標星の選択肢を制限しなければならない。**
制限しないと、Dea Sidus Earring で「19→21」を選べてしまい、**チャート全体が空になる**
(§9.1 の欠損規約が正しく働いた結果として、何も描けない)。ユーザーには「壊れている」と映る。

**実装への要求**:
- `sf_history_items.json` と `/sf-history/equipment` が **`maxStar` を装備ごとに持つ**(SH-3)
- 目標星セレクタは `maxStar` を超える選択肢を**出さない**(SH-5)
- `maxStar` は**ハードコードせず、hourly の実データ(`MAX(item_upgrade)+1`)から導出**する
  (将来ゲーム側で上限が変わっても、データが自動的に追随する)
...
### 9.2.1 ✅ 判定結果 =(a)実挙動(SH-2・統括が独立再計算して一致)

150日分・全28装備で ☆20 と ☆21 の系列を突き合わせた結果:

```
完全一致       :  0 装備   ← 1つも無い
部分的に相違   : 22 装備   一致率 46.7%(Twilight Mark)〜 98.1%(AbsoLab Shining Rod)
比較不能       :  6 装備   ← ☆20/21 のデータthat自体が存在しない(§7.1)
```

**∴ API のクランプではない。** SH-1 が単一時点で見た「☆20/21/22 が同値」は、
動的価格が高星域で**しばしば同じ値に落ち着く**ことの断面であって、構造的な固定ではない。
時系列では全装備で分岐している。

**∴ 高星区間(19→21・21→22)は本物の信号を持つ。** U6 は解決。特別な注記は不要。

> 参考: maplenEnhancebot の parity fixture(別時点で採取)でも ☆16〜22 が同一価格で並んでおり、
> 「ある瞬間には平坦に見える」ことは以前からあった。**瞬間の平坦と構造的な固定は別**という教訓。
...
- **U7 ⬜ 公式 API の真の保持期間**(§2.1)— 160日以上あることは確定したが上限は未解明。
  表示150日には不要なので**追わない**。表示期間を延ばす判断が出たときに初めて調べる
- **U6 ✅ 解決**(§9.2.1)— **(a) 実挙動**。150日×28装備で完全一致は0装備、22装備が時系列で分岐。
  API のクランプではない。高星区間の表示に特別な注記は不要
- **U2 ✅ ルーティング = `#/starforce`(統括裁定 2026-08-05・要朝レビュー)** — 本 SPA は
  **ハッシュルータ**(`src/board/useHashRoute.js`。`#/dashboard` `#/tasks` `#/schedule` `#/character/...`)。
  既存の流儀に合わせる。実パス(`/starforce`)にすると Pages の SPA フォールバック(404.html)設定が要り、
  **配信の根幹を触る**=本設計のスコープを超える。
  - **★正直な但し書き**: ハッシュルートは**検索エンジンに個別 URL として拾われにくい**。
    ∴ §1 で挙げた「SEO 導線」の寄与は、**T5 がハッシュルートのインデックス性をどう扱うかに依存する**。
    T5 が未解決なら、本ツールの寄与は当面**「再訪動機」寄り**であり、検索流入は過大に見積もらない。
    実パス化が要ると判断されたら**別スライス**(配信設定の変更ゆえ、ついでにやらない)
