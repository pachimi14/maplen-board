...
- **TTL 300秒(5分)**のプロセス内キャッシュ(**全ユーザーで共有**・同一 itemId への同時アクセスは
  single-flight で1リクエストに畳む)。ユーザー要求 2026-08-05:「リロードの度にユーザーが取得するのではなく、
  リクエスト時にサーバーが取得し、全ユーザーがそのキャッシュを最大5分間利用する」。
  **構造は元から要求どおりで、変えたのは TTL の 60→300 のみ**。環境変数 `SF_HISTORY_LATEST_TTL_SECONDS` で上書き可
...
- **進行中の区間は 4h テーブルに保存しない**。ジョブ実行時点で未完了の区間は**保存物に出さない**
  (出すと途中値が最終値として固定され、再生成の決定性も壊れる)
- **★ただし表示はする(SH-7・ユーザー裁定 2026-08-05 で §9 を部分的に改訂)** → **§9.3**
- **★★ラベルは区間終了時刻に反転(IMPL_PLAN_SH18・ユーザー裁定 2026-08-05)** — 上記「ラベルは区間開始
  時刻」の文は**書き換えず**、ここに反転として追記する。ユーザー指摘: 「`8/5 0:00` の表記で実際は
  `4:00` 時点の数値というのは非常にわかりにくい」。4時間足の値は区間内**最後**の `endPrice`(直前行
  「代表値」参照)であり、`00:00–04:00` の足の中身は実質「`04:00` 時点の価格」。開始時刻でラベルする
  のは実態とずれている。∴ **足のラベルを区間終了時刻(`T+4h`)にする**。ただし**サーバー・DB・API は
  無改訂**(`price_at` / `labelIs: "bucketStart"` の意味は不変。表示層=ブラウザが `+4h` する)。未終了
  の足(進行中バケット、§9.3)は終了時刻がまだ来ていないため、SH-17 どおり**現在時刻(`asOf`)のまま**
  (未来の時刻を表示しない)。ヒートマップ(§11 UI・実装は `domain/weekdayStats.js`)も同じ規約に揃え、
  セルの割り当てを「区間終了時刻」の曜日・時刻で行う(例: `水20:00–木00:00` の足 → `木`行の`00:00`列。
  結果として左上セルは「木曜 00:00」を維持しつつ中身が1枠ずれる)。統計(`computeStats`/
  `currentPercentile`)はラベルに依存しないため無改訂・無変化。詳細は `docs/reports/SH18_BUCKET_END_LABEL.md`。
...
### 9.3 ★進行中バケットを暫定値として描く(SH-7・ユーザー裁定 2026-08-05)

**問題**: 確定バケットだけを描くと、チャートの右端が現在時刻より **4〜8時間前**で止まる。
ユーザーには「データが古い/壊れている」と映る。

**裁定**: **描く。ただし確定値と見分けられる形で。**

| | 保存(`sf_price_history_4h`) | API 応答 |
|---|---|---|
| 確定バケット | 保存する(決定的・**現状のまま**) | 返す |
| **進行中バケット** | **保存しない** | **`provisional: true` を付けて末尾に1点足す** |

**保存しない理由**: 4h テーブルに書くと SH-3 受け入れ基準 (a)「同じ hourly から2回生成したら完全一致」が壊れる。
**検収で実測**: `/prices` を31回叩いた前後で 4h テーブルが **577,792行・ハッシュ `707de4ad...` で完全一致**。

**値の出どころ = `latest` の共有キャッシュ**(hourly ではない)。hourly は4時間ごとのジョブでしか増えず、
それを使うと暫定点も最大4時間古くなって意味が薄い。`latest` 由来なら**「現在値」カードと同じ数字**になる。
**検収で実測**: 暫定点由来 `283,351,499.41` / latest 由来 `283,351,499.41`(相対差 **9.7e-12** = 丸めのみ)。

**表示**: 最後の確定点→暫定点だけ**破線**(`stroke-dasharray="5 4"`)、暫定点のマーカーは**中抜き**
(`fill="none"`)、ツールチップに「**暫定値(区間未終了)**」。

**★統計からは除外する**(§6.1 と同じ理由)。暫定値は後から動くので、平均・高値・安値・パーセンタイルに
入れるとリロードのたびに数字が変わって再現しなくなる。
**検収で実測**: 暫定点の値を `999,999,999` に改竄しても統計は1ビットも動かない(`count` は確定点のみの 899)。

**`endDate` は確定バケットの最後のまま**。暫定点の時刻は別フィールド `provisionalDate` で返す
(既存フィールドの意味を変えない)。

**★上流失敗時の劣化方向**: `prices` はこれで初めて公式 API に依存する。
**上流が落ちたら暫定点を省いて 200 で確定履歴を返す**(`prices` を 503 にしない=履歴すら読めなくなるのは明白な劣化)。
`latest` 単体は従来どおり上流失敗で 503。
...
最大 900点 × 22値 ≈ raw 250KB。**gzip 実測 86.4KB**(SH-3・上限100KBを充足)。応答 **中央値 32ms**。

### 10.1 応答の数値は小数2桁に丸める(SH-3 の実装判断・統括が実測して承認)

raw 250KB を gzip すると丸めなしでは **127KB** で基準(100KB)を超えたため、
**応答生成時のみ**小数2桁に丸める(**DB の保存値は full precision のまま**)。

**統括が実測した影響**(この数字を根拠に承認した):

| 区間 | full precision | 2桁丸め | 相対差 |
|---|---|---|---|
| 0→22 | 2,630,105,337.26 | 2,630,105,337.50 | **8.9e-11** |
| 19→21 | 879,442,210.72 | 879,442,210.76 | 4.7e-11 |
| 0→17 | 296,249,764.13 | 296,249,764.23 | 3.4e-10 |

**26.3億 NESO に対して 0.24 NESO の差**。UI の表示粒度(`2.63B`)から見て無意味な差であり、
方向性も無い(偶数丸め)。∴ **承認**。
価格そのものの最大相対誤差は **1.26e-5**(最安値 318.66 NESO の星で最大)。

**★ただし「配信値は full precision ではない」ことを README と応答に明示すること。**
後任が full precision を仮定して差分を取ると、原因不明の 1e-10 が出て時間を溶かす。

### 10.2 CORS は img-proxy と同一パターン

**常に許可オリジン文字列(`https://lulumi-tools.com`)を返す**。呼び出し元の `Origin` を反射しない。
許可外のブラウザは自分の origin と一致しないため**ブラウザ側で遮断される**。
これは `server/img-proxy/proxy_core.py:cors_origin_for_request` の踏襲(新パターンを発明しない)。

**★SH-5 への制約**: preflight(`OPTIONS`)は **405** を返す。
∴ **フロントは単純リクエストのみ送ること**(カスタムヘッダを付けない。付けると preflight が走って落ちる)。

### 10.3 最新バケットは暫定値になりうる(既知・自己修復する)

4h の確定条件は **`bucket_end <= now`**(実装済み)。
ただし**公式 API が遅れている**と、時計上は閉じたバケットのデータが未着で、
**その時点の暫定値が最終値として出る**ことがある(設計 §5.2 が8時間の重複窓を持つのはこのため)。

**自己修復する**: 差分ジョブが8時間窓を再取得 → 影響を受けたバケットを再導出する
(SH-3 が「元データが後から修正されるケース」込みでテスト済み)。∴ **1サイクル以内に収束する**。

**SH-6 へ転送する DB の最終バケットは暫定値を1本含む**(バックフィルがバケットの途中で終わったため)。
**初回の update ジョブで自動的に正しい値に置き換わる**ので、転送前の手当ては不要。
...
- **U9 ✅ 解決・★反転(IMPL_PLAN_SH14・ユーザー裁定 2026-08-05)** — SH-11 で「ページ全体を
  閲覧者のローカル時刻に」と決めた表示方針を、本スライスで **UTC に戻した**(曜日の併記は維持、
  曜日も UTC で算出)。反転を支持する統括の実測: 曜日別中央値の開きは UTC 基準 13.9% / JST 基準
  10.4% — **UTC のほうが周期が鋭く出る**(価格周期がゲーム側の UTC 基準リセットに紐づいている
  可能性が高い)。詳細は `docs/reports/SH14_UTC_AND_ORDER.md`。SH-11 の報告書
  (`docs/reports/SH11_LOCAL_TIME_HEATMAP.md`)本文は書き換えず、末尾に同旨の追記のみ行った。
