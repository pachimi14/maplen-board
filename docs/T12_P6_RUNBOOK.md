...
1. **リモートのサイズは減らない**(実測 3,514,447 KB → 3,514,544 KB でむしろ微増)。
   ...
   **【2026-08-17 訂正】誤り**。数時間後の実測で **3,514,447 KB → 4,310 KB(99.88%減)**
   と**自然に縮小した**。PR refs 自体は残り旧コミットも API から取得できるが、
   **GitHub は GC を実施しており PR refs 経由のオブジェクトは `.size` に計上されない**。
   **Support への依頼は不要**(押しても数時間〜1日待てばよい)。
