...
`lulumi-ranking-retry.yml` は cron 3本（`40 0` / `20 1,2`）に対し **毎日2本しか発火していない**。schedule run 20本の conclusion は**全て success で cancelled が1本もない**ため、**concurrency の eviction ではなく GitHub 側の真のドロップ**。~~発火間隔の実測（30〜43分 ≈ cron の40分間隔）から、落ちているのは常に **`20 2`**。~~
**【2026-08-28 訂正】この特定は誤り**。発火時刻の最小遅延から推定したための誤りで、**実際に落ちていたのは先頭の `40 0`（JST 9:40）**だった（過去50本で 00:40〜01:59Z の発火が **0件** であることを直接確認して確定）。つまり**「JST 9:40 の防衛線」は実在しなかった**。詳細と対処は DECISION_LOG **LULU-139**。先行 run がキューに滞留している間に来た cron が捨てられている挙動。
