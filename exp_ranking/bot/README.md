...
| `RANKING_MAX_PAGES` | 2000 | 安全のための最大ページ数（× 10件/ページ = 20,000件）。暴走防止用で通常は到達しない。**到達した場合は `logger.warning` が出る**（2026-07-29〜08-06 に旧上限 800=8,000件で静かに切り捨てが発生したため追加） |

API は総合順位順です。ページを進めるとレベルが下がるため、**そのページの最大レベルが `RANKING_MIN_LEVEL` 未満になった時点で取得を終了**します。

目安:

| 最小レベル | おおよその人数 | ページ数 | 時点 |
|------------|----------------|----------|------|
| 235+ | 約 950 人 | 約 95 ページ | 2026-06 |
| 225+（既定） | 約 5,500 人 | 約 550 ページ | 2026-06 |
| 225+（既定） | **約 8,250 人** | 約 825 ページ | **2026-08**（実測 8,247。約 +25 人/日で増加中） |
