  ...
  return <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/45 p-4 pt-[8vh] backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-labelledby="notification-dialog-title" className="panel-card w-full max-w-4xl shadow-2xl">
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50/70 p-4">
        <label className="flex items-start gap-3 font-semibold text-slate-800">
          <input type="checkbox" className="mt-1" checked={settings.schedule.enabled} onChange={(event) => update("schedule", { enabled: event.target.checked })}/>
          <span><span className="block">{t("notification.schedule")}</span><span className="mt-1 block text-sm font-normal text-slate-500">{t("notification.scheduleHint")}</span></span>
        </label>
      </div>
