...
**スケジュール**（リセット **JST 9:00**。**キューは JST 7 / 8 / 9 時**。実行時は **9:20 まで待機**してから取得・スキップ判定）:

| cron (UTC) | キュー (JST) | 想定 |
|------------|--------------|------|
| `0 22 * * *` | **7:00** | 空きが多いと ~3h で起動 → 9:20 待機後に取得 |
| `0 23 * * *` | **8:00** | 同上・再試行 |
| `0 0 * * *` | **9:00** | 遅延 ~5h なら ~14:00 頃に取得 |

**当日のスナップショットが既に 1000 行以上ある場合**（`schedule` のみ・**9:20 待機のあと**）: API 取得・ビルド・デプロイをすべてスキップ。  
`push` / 手動実行ではスキップしません（待機もしません）。
...
GitHub の schedule は数分〜数時間遅れることがあります（早朝は **~3h**、9時キューは **~5h** 程度の例あり）。**7:00 / 8:00 / 9:00 JST** のいずれかで、最初の 1 回だけ取得し、以降はスキップします。成功ログは **10 分前後**（取得あり）。**20 秒程度**で終わる場合は「当日スキップ」です。それでも足りない場合は **Run workflow** で手動実行してください。
