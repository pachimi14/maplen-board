...
**スケジュール**（リセット **JST 9:00** のあと。**定期実行は必ず 9:20 JST まで待ってから**取得・スキップ判定）:

| cron (UTC) | キュー (JST) | 役割 |
|------------|--------------|------|
| `25 0 * * *` | **9:25** | 本命（取得ウィンドウ 9:20 以降） |
| `30 1 * * *` | **10:30** | 遅延時の再試行 |
| `0 3 * * *` | **12:00** | 同上 |
| `0 6 * * *` | **15:00** | 同上 |
...
GitHub の schedule は数分〜数時間遅れることがあります。**9:25 / 10:30 / 12:00 / 15:00 JST** のいずれかで、遅れて走った最初の 1 回だけ取得し、以降はスキップします。成功ログは **10 分前後**（取得あり）。**20 秒程度**で終わる場合は「当日スキップ」です。それでも足りない場合は **Run workflow** で手動実行してください。
